...
/**
 * This class implements helper routines used to test service hosts in
 * isolation.
 */
public class TestHost extends VerificationHost {

    private static final int WAIT_ITERATION_SLEEP = 500;
    private static final int WAIT_ITERATION_COUNT = 30000 / WAIT_ITERATION_SLEEP; // 30
                                                                                  // seconds.
     ...
     */
    public TestHost(int port, Path storageSandbox, @SuppressWarnings("rawtypes") Class... factoryServices)
            throws Throwable {
        super();
    ...
    public ServiceHost start() throws Throwable {
        super.start();
        for (Class<? extends Service> service : this.factoryServices) {
    ...
    public <T extends ServiceDocument> void patchServiceSynchronously(
        ...
        Operation patchOperation = Operation
                ...
                .setBody(patchBody).setReferer(this.getUri())
    ...
    public <T extends ServiceDocument> T waitForServiceState(Class<T> type,
        ...
        return waitForServiceState(type, serviceUri, test,
                WAIT_ITERATION_SLEEP, WAIT_ITERATION_COUNT);
    }

    public <T extends ServiceDocument> T waitForServiceState(Class<T> type,
            String serviceUri, Predicate<T> test, int waitIterationSleep,
